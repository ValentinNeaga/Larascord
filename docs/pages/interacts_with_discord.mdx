...
The `InteractsWithDiscord.php` trait adds convenience methods to your `User` model that reach through to the linked [DiscordAccount](/discord_account).

<Callout type="info">
    The trait is optional. Larascord works without it, and everything below is also available on the account itself through `DiscordAccount::forUser($user)`.
</Callout>
...
## hasDiscordAccount
<Callout type="info">
    Use this to decide whether to show a "Link Discord" or an "Unlink Discord" button.
</Callout>
```php copy=false
/**
...
## getDiscordTag
<Callout type="info">
    This method will return the user's tag in the format of `username#discriminator`. If the user migrated to the new username system, it will return the user's username, followed by their display name in brackets, for example `jakye (Jakye)`.
</Callout>
```php copy=false
/**
...
<Callout type="warning">
    This method revokes the OAuth2 access token with Discord and deletes the Discord account. The user itself is left alone.
</Callout>
```php copy=false
/**
